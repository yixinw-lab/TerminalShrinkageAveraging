...
def run(runtime: argparse.Namespace) -> Path:
    ...
    def raw_record() -> None:
        ...
        raw_trace.append({"step": ledger.optimizer_steps, **ev, **ledger.as_dict()})
        print(
    ...
    while ledger.charged_flop_equiv < runtime.compute_budget:
        ...
        while eval_idx < len(eval_points) and ledger.charged_flop_equiv >= eval_points[eval_idx]:
            ...
            rows, stats = evaluate_recipes(
                model, optimizer, spec, infos, bank, recipes, pack["eval_batches"],
                token_bytes, device, step=ledger.optimizer_steps, n_layers=n_layers,
            )
            for r in rows:
                r.update({"charged_flop_equiv": ledger.charged_flop_equiv, "trajectory_id": runtime.trajectory_id})
    ...
    theta = spec.flatten_parameters(dtype=torch.float32)
    bank.append(ledger.optimizer_steps, theta, force=True)
    # Always evaluate recipes at the final endpoint if not already done there.
    if not recipe_evals or max(int(r["step"]) for r in recipe_evals) != ledger.optimizer_steps:
        rows, stats = evaluate_recipes(
            model, optimizer, spec, infos, bank, recipes, pack["eval_batches"],
            token_bytes, device, step=ledger.optimizer_steps, n_layers=n_layers,
        )
