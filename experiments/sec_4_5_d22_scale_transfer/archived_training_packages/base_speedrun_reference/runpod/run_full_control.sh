...
activate_repo
need_8_gpus
RATIO="$FULL_CONTROL_RATIO"
RUN_TAG=${RUN_TAG:-og_pr830_r94_$(date +%Y%m%d_%H%M%S)}
...
echo "===== OG PR830 FULL CONTROL: ratio $RATIO, no clamp, raw ====="
cd "$REPO"
...
python "$SUITE_DIR/tools/summarize_arm.py" --arm og_full_control --result-dir "$RESULT_DIR" --threshold "$CORE_THRESHOLD"
